...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const log = await getLogger({ userId: session.user.id, serventiaId: membro.serventiaId, action: 'gerar_ata_restauracao' })
    log.error({ err, testeId: id }, 'Falha inesperada ao gerar ata de teste de restauração')
