...
export async function PATCH(
  ...
  try {
    ...
    const log = await getLogger({ userId: session.user.id, action: 'atualizar_evidencia' })
    log.error({ err, evidenciaId: id }, 'Falha inesperada ao atualizar metadados de evidência')
...
export async function DELETE(
  ...
  try {
    ...
    const log = await getLogger({ userId: session.user.id, action: 'excluir_evidencia' })
    log.error({ err, evidenciaId: id }, 'Falha inesperada ao excluir evidência')
    return NextResponse.json({ error: 'Erro interno. Tente novamente em instantes.' }, { status: 500 })
  }
}
