...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const log = await getLogger({ userId: session.user.id, serventiaId: membro.serventiaId, action: 'exportar_hashlist' })
    log.error({ err }, 'Falha inesperada ao gerar a lista de hashes do dossiê')
