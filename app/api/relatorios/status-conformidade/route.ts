...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const log = await getLogger({ userId: session.user.id, serventiaId: membro.serventiaId, action: 'gerar_status_conformidade' })
    log.error({ err }, 'Falha inesperada ao gerar relatório de status de conformidade')
