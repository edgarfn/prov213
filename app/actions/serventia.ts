...
export async function criarServentia(formData: FormData) {
  ...
  try {
    ...
    const log = await getLogger({ userId, action: 'criarServentia' })
    log.error({ err }, 'Falha inesperada ao criar serventia')
...
export async function atualizarServentia(serventiaId: string, formData: FormData) {
  ...
  try {
    ...
    const log = await getLogger({ userId, serventiaId, action: 'atualizarServentia' })
    log.error({ err }, 'Falha inesperada ao atualizar serventia')
    return { error: 'Erro interno. Tente novamente em instantes.' }
  }
}
...
export async function alternarAtivaServentia(serventiaId: string, ativa: boolean) {
  ...
  try {
    ...
    const log = await getLogger({ userId, serventiaId, action: 'alternarAtivaServentia' })
    log.error({ err }, 'Falha inesperada ao ativar/inativar serventia')
    return { error: 'Erro interno. Tente novamente em instantes.' }
  }
}
