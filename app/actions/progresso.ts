...
export async function atualizarProgresso(
  ...
  try {
    ...
    const log = await getLogger({ userId, serventiaId, requisitoId, action: 'atualizarProgresso' })
    log.error({ err }, 'Falha inesperada ao atualizar progresso de requisito')
...
export async function declaraConclusaoEtapa(serventiaId: string, etapaId: string) {
  ...
  try {
    ...
    const log = await getLogger({ userId, serventiaId, etapaId, action: 'declaraConclusaoEtapa' })
    log.error({ err }, 'Falha inesperada ao declarar conclusão de etapa')
    return { error: 'Erro interno. Tente novamente em instantes.' }
  }
}
