...
const ACOES_LABELS: Record<string, string> = {
  ...
  USUARIO_ATUALIZADO: 'Dados do usuário atualizados',
  USUARIO_ATIVADO: 'Acesso do usuário reativado',
  USUARIO_DESATIVADO: 'Acesso do usuário desativado',
  USUARIO_REMOVIDO: 'Usuário excluído permanentemente',
...
export function AuditoriaClient() {
  ...
  const fetchLogs = useCallback(async (pg = 1) => {
    ...
    if (!res.ok) {
      setErro(d.error ?? 'Não foi possível carregar o registro de auditoria.')
      setDados(null)
      setLoading(false)
      return
    }
